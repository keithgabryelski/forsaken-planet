...
export default Kapsule({
  props: {
    width: { default: 1024 /*window.innerWidth*/ },
    height: { default: 1024 /*window.innerHeight*/ },
    data: {
      onChange(_, state) {
    ...
    color: { default: (_d) => "lightgrey" },
    strokeColor: { default: (_d) => "white" },
    ...
    tooltipContent: { default: (_d) => "", triggerUpdate: false },
    tooltipTitle: { default: null, triggerUpdate: false },
    showtooltip: { default: (_d) => true, triggerUpdate: false },
    focusOnNode: {
  ...
  update: function (state) {
    ...
    state.radiusScale.range([
      maxRadius * Math.max(0, Math.min(1, state.centerRadius)),
      maxRadius,
    ]);
    ...
    state.svg
      ...
      .attr(
        "viewBox",
        `${-state.width / 2} ${-state.height / 2} ${state.width} ${state.height}`,
      );

    if (!state.layoutData) return;

    const focusD = (state.focusOnNode &&
      state.focusOnNode.__dataNode.y0 >= 0 &&
      state.focusOnNode.__dataNode) || { x0: 0, x1: 1, y0: 0, y1: 1 };

    const slice = state.canvas.selectAll(".slice").data(
      state.layoutData.filter(
        (
          d, // Show only slices with a large enough angle and within the max levels
        ) =>
          d.x1 > focusD.x0 &&
          d.x0 < focusD.x1 &&
          (d.x1 - d.x0) / (focusD.x1 - focusD.x0) > state.minSliceAngle / 360 &&
          (!state.maxLevels ||
            d.depth - (focusD.depth || (state.excludeRoot ? 1 : 0)) <
              state.maxLevels) &&
    ...
    // Apply zoom
    try {
      state.svg.transition(transition).tween("scale", () => {
        const xd = d3Interpolate(state.angleScale.domain(), [
          focusD.x0,
          focusD.x1,
        ]);
    ...
    // angular label
    const angularLabel = newSlice.append("text").attr("class", "angular-label");
    ...
    const computeAngularLabels =
      state.showLabels &&
      ["angular", "auto"].includes(state.labelOrientation.toLowerCase());
    const computeRadialLabels =
      state.showLabels &&
      ["radial", "auto"].includes(state.labelOrientation.toLowerCase());
    ...
    function middleArcLine(d) {
      ...
      const angles = [
        state.angleScale(d.x0) - halfPi,
        state.angleScale(d.x1) - halfPi,
      ];
      const r = Math.max(
        0,
        (state.radiusScale(d.y0) + state.radiusScale(d.y1)) / 2,
      );
    ...
    function radialTextTransform(d) {
      const middleAngle =
        (state.angleScale(d.x0) + state.angleScale(d.x1) - Math.PI) / 2;
      const r = Math.max(
        0,
        (state.radiusScale(d.y0) + state.radiusScale(d.y1)) / 2,
      );
    ...
    function angularTextFits(d) {
      return (
        measureTextWidth((nameOf(d.data) ?? "").toString(), TEXT_FONTSIZE, {
    ...
    function radialTextFits(d) {
      const availableHeight =
        state.radiusScale(d.y0) *
        (state.angleScale(d.x1) - state.angleScale(d.x0));
    ...
    function autoPickLabelOrientation(d) {
      ...
      const angle =
        ((state.angleScale(d.x0) + state.angleScale(d.x1)) / 2) % Math.PI;
